    ...
    try {
      ...
      if (rentCost > 0) {
        const rentTransaction = new AccountingTransaction({
          ...
          category: 'booth_rent',
          amount: rentCost,
          description: `ค่าเช่าหน้าร้าน ${location} (${new Date(startDate).toLocaleDateString('th-TH')} - ${new Date(endDate).toLocaleDateString('th-TH')})`,
          boothId: booth._id,
          relatedId: booth._id,
        ...
        for (const employee of employees) {
          if (employee.salary > 0) {
            ...
            const salaryTransaction = new AccountingTransaction({
              ...
              amount: totalSalary,
              description: `ค่าจ้างพนักงาน ${employee.name} (฿${employee.salary}/วัน × ${numberOfDays} วัน)`,
              boothId: booth._id,
              relatedId: booth._id,
              relatedType: 'booth_setup',
              brandId: payload.user.brandId
      ...
      if (businessPlan?.equipmentId && businessPlan?.fixedCosts?.equipment > 0) {
        ...
        const equipmentTransaction = new AccountingTransaction({
          ...
          description: `ค่าอุปกรณ์หน้าร้าน`,
          boothId: booth._id,
          relatedId: booth._id,
          relatedType: 'booth_setup',
          brandId: payload.user.brandId
